...
################################# load model
model_name_or_path = f"../../llama2/Llama-2-13b-chat/"
model = AutoModelForCausalLM.from_pretrained(model_name_or_path, torch_dtype=torch.float16, device_map="auto", token=True).eval() 
use_fast_tokenizer = "LlamaForCausalLM" not in model.config.architectures
tokenizer = AutoTokenizer.from_pretrained(model_name_or_path, use_fast=use_fast_tokenizer, padding_side="left", legacy=False, token=True)
tokenizer.pad_token_id = 0 if tokenizer.pad_token_id is None else tokenizer.pad_token_id
...
for j, instruction in enumerate(raw_test_data[1:20:2]):
    ...
    with torch.no_grad():
        ...
        # logits_gt_20_values, logits_gt_20_indices = torch.topk(logits_gt.to(dtype=torch.float32), 20)
        # p = torch.nn.functional.softmax(logits_gt,dim=0)
        # outputs = model.generate(prompt.cuda(), max_new_tokens=256, do_sample=False).detach().cpu()
        # generation_gt = tokenizer.decode(outputs[0], skip_special_tokens=False).replace(instruction, "")
        # generation = tokenizer.decode(outputs[0], skip_special_tokens=False).replace(instruction, "")
    with open(f'separation_by_dr/logits_iter_gt_inst_{j}.json', 'w') as file:
        json.dump(logits_gt.tolist(), file)
    # with open(f'generations_multi_choice_harm/generation_iter_gt_inst{j}.json', 'w') as file:
    #     json.dump(generation_gt, file)
...
for j, instruction in enumerate(raw_test_data[1:20:2]):
    ...
    for i, coeff in tqdm(enumerate(x), total=len(x)):
        ...
        with torch.no_grad():
            ...
            # logits_20_values, logits_20_indices = torch.topk(logits.to(dtype=torch.float32), 20)
            # tokens = [tokenizer.convert_ids_to_tokens(index) for index in logits_20_indices]
            # p = torch.nn.functional.softmax(logits,dim=0)
            # logits_dr = logits - logits_gt
            # res_answer = logits[possible_answers]
            # prediction = torch.argmax(res_answer).cpu().item() + 1
            # outputs = model.generate(prompt.cuda(), max_new_tokens=128, do_sample=False).detach().cpu()
            # generation = tokenizer.decode(outputs[0], skip_special_tokens=False).replace(instruction, "")
        # with open(f'generations_multi_choice_harm/generation_iter_{i}.json', 'w') as file:
            # json.dump(f'{generation}', file)
