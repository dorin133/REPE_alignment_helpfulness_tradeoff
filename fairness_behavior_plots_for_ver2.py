...
for model_name in ["Llama-2-13b", "Llama-2-13b-chat"]:
    ...
    for i, coeff in enumerate(x):
        ...
        behavior_bias_std[coeff] = np.std(behavior_bias)
        # behavior_unbias_mean[coeff] = sum(behavior_unbias)/min(len(behavior_unbias), 100)
        ...
        # print(f'p_mean_relative for coeff = {coeff}: {behavior_unbias_mean[coeff]}')

    with open(f'../../lab_data/mmlu_plots_correction/behavior_plots/behavior_bias_{model_name}_fairness.json', 'w') as file:
