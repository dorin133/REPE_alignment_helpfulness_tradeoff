...
for dataset_name in dataset_names: # , 'high_school_computer_science', 'medical_genetics', 'international_law', 'clinical_knowledge'
    ...
    for coeff in x:
        ...
        acc_std[coeff] = np.mean(np.std(np.nan_to_num(acc_answer_samples, nan=0.0), axis=0))
        ...
        # projection_on_delta()
        with open(f'{args.output_dir}/{dataset_name}/helpfulness_fairness_{args.model_name.replace("/","_")}_proj.pkl', 'wb') as f:
