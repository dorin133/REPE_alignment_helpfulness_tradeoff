...
args = fairness_utils.GenerationArgsHelpfulness()
...
if args.model_name == 'llama-2-13b':
    if args.bias == 'harmfulness':
        train_data, train_labels, _, _ = fairness_utils.reading_vec_dataset_Q_and_A()
    else:
        bias_data = fairness_utils.bias_dataset_for_alignment(user_tag=user_tag, assistant_tag=assistant_tag, alignment=True, bias=args.bias)
        train_dict = bias_data['train']
        train_data, train_labels = train_dict['data'], train_dict['labels']
else:
    if args.bias == 'harmfulness':
        train_data, train_labels, _, _ = fairness_utils.reading_vec_dataset_by_github()
    else:
        bias_data = fairness_utils.bias_dataset_for_alignment(user_tag=user_tag, assistant_tag=assistant_tag, alignment=False, bias=args.bias)
        train_dict = bias_data['train']
        train_data, train_labels = train_dict['data'], train_dict['labels']
...
for mmlu_dataset_name in ['international_law', 
    ...
    wrapped_model.reset()
    logits_dict, best_inds_dict = fairness_utils.get_no_repe_results(wrapped_model, tokenizer, dataset)

    # test model on dataset for various norms of injected vectors
    x = [i for i in np.arange(args.start_coeff, args.end_coeff, 0.5)]
    ...
    for i, coeff in enumerate(x):
        ...
        with open(f'./data/mmlu_plots/helpfulness_plots/{mmlu_dataset_name}_{args.model_name}_fairness.json', 'a') as file:
        ...
        print(f'model: {args.model_name}, dataset: {mmlu_dataset_name}, bias: {args.bias}, coeff: {coeff}')
        print('-----------------------------------')
