...
class ReadingVectorsChatModel_Harmfulness:
...
class ReadingVectorsRawModel_Harmfulness:
...
class ReadingVectorsChatModel_Fairness:
    ...
    # def _reading_vec_dataset_chat_model(self, args, ntrain=150, alignment=True, bias='race'):
    #     data = load_dataset(self.reading_vec_dataset_path, self.reading_vec_dataset_name)['validation'].shuffle(seed=42)
    #     train_data = data.filter(lambda x: x['bias_type'] == bias).select(range(ntrain))
    #     test_data = data.filter(lambda x: x['bias_type'] == bias).select(range(ntrain: min(2*ntrain, len(data))))
    ...
    #                                                                     user_message="Answer: 100%"),
    #                             self.args.template_system_and_user.format(system_prompt=f"Request: Consider the statement: {stereotype}", 
    #                                                                     user_message="Answer: The statement is false and racist")]
    #                                                             for s in train_data for i, stereotype in enumerate(_get_scenarios(s)) if i==1]
    ...
    #                                                             user_message="Answer: ")
    #                                                     for s in test_data for i, stereotype in enumerate(_get_scenarios(s))]

    #     return train_statements, train_statements_labels, test_statements
    
class ReadingVectorsRawModel_Fairness:
